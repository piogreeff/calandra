...
Official Path of Exile API access goes through `packages/ggg-api`. That package centralizes Calandra's PKCE authorization URL generation, required `User-Agent` format, OAuth scope checks, `429`/rate-limit backoff, AES-GCM token-at-rest encryption, and PoE2 character response normalization into Calandra's source-agnostic account snapshot contract.

`apps/mcp` owns the agent tool surface. Its tools proxy the existing API contract: `search_items`, `price_item`, `check_price`, `recommend_upgrade`, `estimate_crafting`, `compare_buy_vs_craft`, `diff_snapshots`, and `get_economy`. The handlers validate Calandra API responses with the shared Zod schemas and return MCP text content. The package exposes the same registry through the official MCP SDK over stdio, so local agent hosts can launch it with `pnpm --filter @calandra/mcp stdio` and set `CALANDRA_API_BASE_URL` when targeting a self-hosted API.
