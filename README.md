...
The Worker serves the current contract at `/openapi.json`; during temporary-domain development that is `https://calandra-api.piogreeff.workers.dev/openapi.json`. Published dataset metadata is available at `/datasets/manifest?league=<league>&patch=<patch>` after the Worker validates the artifact checksum and counts. Ladder browsing is served from `/builds/ladder?league=<league>&patch=<patch>&className=<class>&skill=<skill>&limit=<n>`.
...
Official GGG API calls should go through `packages/ggg-api`, which builds PKCE authorization URLs, enforces Calandra's descriptive `User-Agent`, OAuth scope checks, `429`/rate-limit backoff, AES-GCM token-at-rest encryption, and source-agnostic PoE2 character snapshot capture.
...
pnpm dataset:import -- --cache-icons --artifact ./data/dawn-0.2.0.json --icon-cache-dir ./dist/icons --execution-context maintainer
pnpm dataset:import -- --publish-icon-cache --icon-cache-dir ./dist/icons --icon-cache-manifest ./dist/icons/icon-cache.manifest.json --r2-bucket calandra-data --execution-context maintainer
...
The `--maintainer-normalized` path is maintainer-only and turns already collected poe2db/poe.ninja records into a patch-versioned artifact; it still does not put scrapers in clients or self-host installs. `--cache-icons` is also maintainer-only: it reads artifact `iconSourceUrl`/`iconCacheKey` metadata, writes local cached image files, and emits an attribution manifest without bundling game art in the repo. `--publish-icon-cache` uploads those cached files and the attribution manifest to R2 with Wrangler. Self-host imports consume a published artifact only. With `--manifest`, imports verify counts and SHA-256 before use. With `--publish-dir`, it writes the same `datasets/{league}/{patch}.json` layout that the Worker reads from R2 plus a sibling `datasets/{league}/{patch}.manifest.json` containing counts and a SHA-256 checksum. Maintainer publishing can pass `--r2-bucket` to upload both remote R2 objects with Wrangler after validation. `--expected-unique-count` and `--minimum-unique-image-coverage` enforce the Phase 1 unique-image gate before an artifact is published. The Worker validates that manifest before serving R2-backed dataset responses.
