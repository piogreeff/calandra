...
That path consumes a published artifact only; self-host installs do not run scrapers. With `--manifest`, imports verify counts and SHA-256 before use. With `--publish-dir`, it writes the same `datasets/{league}/{patch}.json` layout that the Worker reads from R2 plus a sibling `datasets/{league}/{patch}.manifest.json` containing counts and a SHA-256 checksum. Maintainer publishing can pass `--expected-unique-count` and `--minimum-unique-image-coverage` to enforce the Phase 1 unique-image gate before an artifact is published. The Worker validates that manifest before serving R2-backed dataset responses.
