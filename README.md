...
The `--maintainer-normalized` path is maintainer-only and turns already collected poe2db/poe.ninja records into a patch-versioned artifact; it still does not put scrapers in clients or self-host installs. `--cache-icons` is also maintainer-only: it reads artifact `iconSourceUrl`/`iconCacheKey` metadata, writes local cached image files, and emits an attribution manifest without bundling game art in the repo. Self-host imports consume a published artifact only. With `--manifest`, imports verify counts and SHA-256 before use. With `--publish-dir`, it writes the same `datasets/{league}/{patch}.json` layout that the Worker reads from R2 plus a sibling `datasets/{league}/{patch}.manifest.json` containing counts and a SHA-256 checksum. Maintainer publishing can pass `--r2-bucket` to upload both remote R2 objects with Wrangler after validation. `--expected-unique-count` and `--minimum-unique-image-coverage` enforce the Phase 1 unique-image gate before an artifact is published. The Worker validates that manifest before serving R2-backed dataset responses.
